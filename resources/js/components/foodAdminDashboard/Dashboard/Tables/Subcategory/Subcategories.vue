...
export default class subcategories extends Vue {
  ...
  public deleteItemConfirm(): void {
    ...
    axios
      .delete("http://127.0.0.1:8000/api/Subcategory/del/" + this.Id)
      .then(Response => {
  ...
  public save(): void {
    if (this.editedIndex > -1) {
      ...
      axios
        .put(
          "http://127.0.0.1:8000/api/Subcategory/upd/" + this.Id,
          this.editedItem
        )
        .then(Response => {
          if (Response.data.data == true) {
      ...
      axios
        .post("http://127.0.0.1:8000/api/Subcategory/create", this.editedItem)
        .then((Response) => {
          if (Response) {
            alert("Inserted");
          } else {
        ...
        axios.get("http://127.0.0.1:8000/api/Subcategory/show").then(Response => {
      this.data = Response.data.Subcategory;
    });
    }
  ...
  mounted() {
    ...
    axios.get("http://127.0.0.1:8000/api/category/show").then((Response)=>{
        Response.data.category.forEach((element:any) => {
          this.idss.push(element['id'])
        });
        })
        console.log("Idssss",  this.idss)
